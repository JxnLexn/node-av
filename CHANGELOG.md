...
- **FMP4 streams stop on large backward DTS jumps instead of accumulating native muxer packets** ([#351](https://github.com/seydx/node-av/issues/351)). `maxDtsCorrection` defaults to 1,000,000 microseconds; a larger correction ends the session through `onClose(error)` so its owner can recreate a synchronized A/V stream. `maxInterleaveBytes` additionally caps the native interleaving queue at 64 MiB independently of media timestamps. Both options accept zero to disable; generic `Muxer` instances retain their previous behavior unless opted in. Low-level `FormatContext.interleavedWriteFrame()` and its synchronous variant accept the byte budget as an optional second argument. The limit includes backing buffers, side data and packet metadata, not codec/container-private buffering. FMP4 cleanup now releases input/codecs and delivers the original error even if closing a failed muxer rethrows its worker error.
